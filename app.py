...
def rice_disease():
    ...
    return render_template("rice_disease.html", user=current_user(), prediction=prediction, confidence=confidence, filename=filename)
...
def soil_data():
    data = {
        "nitrogen": round(random.uniform(0, 200), 2),
        "phosphorus": round(random.uniform(0, 200), 2),
        "potassium": round(random.uniform(0, 200), 2),
    }
    return jsonify(data)
